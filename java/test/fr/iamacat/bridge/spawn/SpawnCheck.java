...
public final class SpawnCheck {
    ...
    private static Snapshot seal(SpawnStore store, long tick) {
        ...
        Map<MatouId, Object> states =
                SpawnSeal.seal(vocab(), store, MOB,
                        wired.cap(BEAST_SHORT),
                        wired.budget(BEAST_SHORT),
                        wired.yMin(BEAST_SHORT),
    ...
    public static void main(String[] args) {
        ...
        // Forge dispatch stays single-mob (ports tranche TODO): the
        // sole view refuses on 2-mob content, loudly — never silent.
        // This documents why the dispatch port is needed.
        final SpawnTable refusalWired = wired;
        expectIAE(new Runnable() {
            @Override public void run() { refusalWired.mob(); }
        }, "sole-view mob on 2-mob content");
        ...
        expectIAE(new Runnable() {
            @Override public void run() {
                OperatorPolicy.effectiveSpawn(contentCap.cap(BEAST_SHORT),
                        contentCap.budget(BEAST_SHORT),
                        contentCap.yMin(BEAST_SHORT),
