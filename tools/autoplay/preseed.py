...
1.16.5 shape (measured, not assumed): the legacy top-level generatorName
/ generatorOptions / RandomSeed tags are IGNORED once WorldGenSettings
exists — a preseed without it silently generates default noise terrain
(caught live by the verdict: andesite/ores/dirt in the slices). The
WorldGenSettings below mirrors a real flat dedicated-server level.dat
(bedrock/dirt/grass layers, no lakes, no features, EMPTY structures so no
village piece can ever land in the proof slices), with seed 0.
