...
public class AutoplayMod {
    ...
    private void spawnTick() {
        ...
        if (total > maxBeasts) {
            maxBeasts = total;
            System.out.println("[MatouAutoplay] spawn census <" + total
        ...
        if (total > SPAWN_CAP) {
            spawnFail("cap breached <" + total + " > " + SPAWN_CAP
                    + "> at worldTick " + worldTicks);
            return;
        }
        if (beasts > 4) {
     ...
     * it, and an authorless strike proves nothing. 1.16.5 shape: no
     * {@code playerEntities} field ships, so the list comes from
     * {@code ServerWorld.getPlayers} (same shape as the loot/spike
     * player helpers).
     */
    private PlayerEntity combatPlayer() {
        if (!(world instanceof ServerWorld)) {
            if (!playerNoted) {
                playerNoted = true;
                System.out.println("[MatouAutoplay] note combat-proof : "
                        + "non-server world at strike tick, postponing");
            }
            return null;
        }
        List<? extends PlayerEntity> players =
     ...
     * members go through the declaring stub type, never the beast —
     * {@code Entity} for positions/eye/teleport, {@code PlayerEntity}
     * for the strike itself (declared there), {@code LivingEntity}
    ...
    private void combatAttack(MatouEntity beast, String mob) {
        int strikeTick = "my_brute".equals(mob) ? COMBAT_TICK_BRUTE
                : COMBAT_TICK;
        PlayerEntity player = combatPlayer();
        if (player == null) {
        ...
        double bx = body.getPosX();
        double by = body.getPosY();
        double bz = body.getPosZ();
        Entity pbody = player;
