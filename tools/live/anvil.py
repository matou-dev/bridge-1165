#!/usr/bin/env python3
"""E3 probe: list block names at height y in chunk (cx,cz) of a 1.16.5
region file (post-flattening palette under Level.Sections: each section
carries a Palette of namespaced names plus packed BlockStates longs)."""
...
def main():
    ...
    found = {}
    # 1.16.5 nests sections under Level (the top-level "sections" layout
    # only arrives in 1.18); names are Palette/Name + BlockStates.
    for sec in root["Level"]["Sections"]:
        base = sec["Y"] * 16
        if base <= y < base + 16:
            if "Palette" not in sec:
                continue  # All-air section: Mojang omits palette + data.
            palette = [e["Name"] for e in sec["Palette"]]
            if "BlockStates" in sec:
                idx = unpack_palette(palette, sec["BlockStates"])
            else:
